...
- **2 = vídeos MP4** (lips-sense-avancado + `-2`): 4 vídeos (`mofu-1/2`, `bofu-1/2`, 69–82 MB) servidos de `jayacademy.com.br/wp-content/uploads/2026/04/`. **⚠️ PENDENTE — único bloqueador restante.** Decisão do usuário: **host de vídeo externo** (Vimeo/Cloudflare Stream). Tentativa de subir pro storage S3 atual FALHOU: Supabase free tier limita **50 MB/arquivo** (413 EntityTooLarge). Vídeos baixados pra `videos-lips-sense-backup/` (gitignored) pra handoff. Falta: usuário subir no host externo → passar as URLs → reescrever as 2 páginas no KV (trocar `<video src>` pelo embed). One-shot `?fixlipsvideos=1` (fases `&one=<url>`/`&rewrite=1`) existe mas só serve pra storage próprio; pro host externo a reescrita será manual/adaptada.
