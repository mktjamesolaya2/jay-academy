...
- **Copy:** skill `copy-pagina-evento-pago` (zip em `tmp/03-skill-de-criacao-de-pagina.zip`), caminho "usar a
  página atual". 9 seções; mecanismo nomeado **"los 4 Controles del Trazo"** (dirección, inclinación, profundidad,
  presión). O "de U$ 297" saiu (desconto não comprovado). DOCX em `tmp/copy-inmersion-pelo-a-pelo.docx`.
...
- ⚠️ **Pendente de confirmação do James:** o conteúdo do PDF (os 4 materiais têm nome criado a partir do programa),
  a frase "enseña a profesionales de distintos países" (tirada das bandeiras da v1). Plataforma confirmada: **YouTube**.
  A v1 fica como está (sem redirect), por decisão do James.

---

## 💋 Protocolo Labial — a página do evento de R$ 27 (24/09)

Evento pago ao vivo (Zoom, 9h–13h, sala fechada) com copy em DOCX (`tmp/copy-protocolo-labial-do-escuro-ao-rosado.docx`).
Página em `lp-html/protocolo-labial.html`, rota `app/protocolo-labial/route.ts`, registry com
`assetsDir: public/lp/protocolo-labial`, Pixel ligado (`PIXEL_SLUGS` + teste). **noindex** e fora do sitemap.

- **Identidade:** o James mandou como base a LP Lovable `premium-craft-05` ("Pele Limpa em 90 Dias") e pediu
  "usar tudo dela" — estrutura, carrosséis, animações — com o olympo **só em toque** (a 1ª versão, escura com
  arco/colunas/pergaminho, foi descartada a pedido). Neutros da referência (`#fdfcf8`, tinta `#0d0806`),
  acento ember trocado por rosa `#c4586f`, Instrument Serif + Inter + mono do sistema. Olympo = greca dourada
  fina sob 3 títulos, numerais romanos na timeline, anel dourado no preço.
- **Reimplementado em vanilla** (a referência é React + framer-motion + Embla): coverflow de 5 cards (os 5
  materiais; centralizado −2..+2, a referência é assimétrica), carrossel de fotos com drag/snap e bolinha que
  vira pílula (resultados), timeline (programação), bento 6 tiles (ao vivo), card com o R$ 27 no disco (valor),
  sliders antes/depois por `clip-path`, parallax num loop de rAF, reveals por IntersectionObserver.
  `prefers-reduced-motion` desliga tudo.
- **Regras da copy:** nenhum logo; R$ 27 grande no hero (1ª dobra do celular: y 306–392 em 390×844), no disco
  do valor e na oferta; barra fixa no celular.
- **Seções da referência sem conteúdo ficaram de fora** (depoimentos, bônus, garantia, FAQ) — não inventar.
- **Assets:** `resultados/01–10.webp` (antes/depois 4:5, vindos da Lips Sense avançado no Supabase + wpmirror,
  sem a imagem com logo Lips Sense), `antes-depois/NN-{antes,depois}.webp` (as compostas cortadas ao meio pro
  slider), `james.webp` (recorte transparente), `procedimento.webp`, `james-avaliando.webp`.
...
assets são os da v2 (`/lp/fio-a-fio-realista-v2/`, nada duplicado): muda a copy e a ordem.

- **Hero**: sem logo. Promessa "Faça o fio que continua realista depois da cicatrização" e
  **preço embaixo do botão** (12x R$ 20,37 / R$ 197). No celular de 844px o preço cabe na 1ª tela.
- **Seções novas**: diagnóstico "Por que o seu fio… desaparece em 30 dias" (inimigo: a
  corrida pelo fio fino; mecanismo: **Ponto de Ancoragem**), "Você vai se reconhecer aqui"
  (é / não é para você), alunas do curso online, bônus nomeados, "Por que custa R$ 197".
- **Alunas**: só entram comentários que dizem **curso online** explicitamente (artifact
  "Depoimentos de Alunos", 2ª busca). **Nenhum cita o Fio a Fio**, então a moldura diz
  "alguns em outros cursos da Academy". Trechos cortados levam "(…)". ⚠️ Falta pedir
  autorização às 4 (@pallomamarrafonsobrancelhas, @pmu_lumoreira, @jamilesantostudio,
  @rosemoreiradesigner) antes de a página virar a oficial.
- Os 4 logos saíram, junto com "vaga" e "Condição promocional". Os CTAs ficaram em dois textos.
  Cada módulo ganhou "VOCÊ SAI SABENDO". Bio com o release em 1ª pessoa. FAQ +6 (medo).
- Dados do James (23/09): cupom da Jayloja = **10%** (bônus 03); suporte = **grupo de
  WhatsApp** (entrou na lista da oferta e no FAQ). A seção com foto de trabalho de aluna do
  Fio a Fio foi **descartada** por ora: não há fotos.

---

## 📣 O Encontro de Especialistas virou página — e o destino é o grupo (16/09)

`jayacademy.com.br/encontro-de-especialistas` foi ao ar em dois passos no mesmo dia:
primeiro só um "Em breve", pra o endereço já poder ser divulgado (commit `e4b55cb`),
depois a LP inteira (commit `eea5d00`).

**O que a página é.** O destino de um QR Code impresso — quem chega escaneou, não
buscou. Por isso ela é curta, mobile-first e continua `noindex` e fora do sitemap.
A promessa é um talk show online com três aulas: James na técnica, **Dra. Rafaela**
no jurídico da micropigmentação e **Diana** em visão de negócios. A conversão **não
é** a inscrição no encontro: é entrar no grupo de WhatsApp, onde a data é anunciada e
a lead é aquecida até lá.

**Visual.** Mesmo sistema da `/academy` no osso (~200 linhas de CSS): Poppins 200/300/400,
caixa alta com `letter-spacing` alto, filete de 54px antes do parágrafo, alternância
preto `#070605` / off-white. Sem o lettering da abertura — numa página de QR Code ele
só atrasa a primeira tela.

⚠️ **Duas molduras vazias esperando foto leem como imagem quebrada.** A primeira versão
tinha os três especialistas em cards com retrato, e só o James tem foto no repo: ao lado
dele, os quadros da Rafaela e da Diana pareciam erro de carregamento. O retrato do James
subiu pra dobra da promessa (uma `dupla`, como na `/academy`) e os três cards ficaram só
tipográficos — iguais entre si. Quando as fotos chegarem (`public/lp/encontro/`), os três
ganham moldura juntos; o HTML marca o ponto.

⚠️ **A hero do celular precisa de `height` de verdade, não `min-height`.** Sem a linha
`.hero { height: 100svh }` do bloco mobile, o `height:100%` do palco e da foto não
resolvem, a imagem colapsa em zero e a primeira tela fica só com o degradê preto.

**Formulário.** Nome, WhatsApp, e-mail e @instagram, todos obrigatórios, no contrato de
sempre: `class="elementor-form"` + `action` pra própria página, que é o que faz a ponte
injetada pelo `serveLp` mandar pro `/api/elementor-form` (CRM + inbox `/leads`). Máscara
e validação de telefone copiadas do `jaytransforma-beauty` (espelho de `lib/telefone.ts`),
mais uma normalização do Instagram: URL colada da barra do navegador vira `@perfil`.
A chave do CRM é a padrão do site — se o James quiser separar estes leads, cola o bloco
do CRM em `/lps/encontro-de-especialistas`.

**Os grupos de WhatsApp saíram da rota.** O mapa de destino padrão morava dentro do
`app/api/elementor-form/route.ts` com só a `transforma` dentro. Virou
`GRUPO_WHATSAPP_POR_LP`, em `lib/lp-funil.ts`, ao lado dos checkouts e das regras que
precisam andar junto com eles. O grupo deste encontro entra **vazio de propósito**: sem
link, a cascata devolve `null`, a pessoa vê a confirmação e fica na página. Quando o
grupo existir, é uma string e um push.

**Teste que melhorou.** O "REGRAS_POR_LP não tem entrada órfã" dizia `slug === "transforma"`
— teria que ser editado a cada página de captação nova. Agora pergunta o que importa: ou
a página termina num checkout, ou termina num grupo. Mais três casos novos sobre os
grupos (convite de grupo de verdade, destino vazio que não vira redirect quebrado, e
nenhum link repetido entre páginas). 496 testes, `tsc` limpo.

**Verificado no navegador de verdade** (headless, 390px): máscara escrevendo
`+55 (19) 99163-4567`, `https://www.instagram.com/jayo.academy/?hl=pt` virando
`@jayo.academy`, submit gravando o lead em `/leads` com o telefone em E.164 e o
`instagram` nas respostas, e nenhum redirect (grupo ainda vazio). Leads de teste apagados.

**Pendências do James:** link do grupo, fotos da Dra. Rafaela e da Diana, data do
encontro. Nenhuma bloqueia a página.

---

## 🔁 O "Reenviar" chegava no CRM pela metade — e nem estava na tela (10/09)

Dois problemas desde 13/08, quando o botão foi criado.

**1. O corpo do reenvio era mais pobre que o da captura.** `reenviarProCrmAction`
montava o JSON à mão com `nome/email/telefone/pagina`. As respostas de qualificação e a
`tag: "JAY Transforma"` ficavam pelo caminho — no botão que existe justamente pra
recuperar lead que não chegou ao CRM, o comercial recebia um contato sem nada que
dissesse por onde começar. O dado nunca esteve perdido: `lead.respostas` guarda o
`fields` original da captura.

**A causa é a mesma de sempre: regra duplicada.** A escolha de qual corpo mandar
(Transforma x resto) morava só dentro de `app/api/elementor-form/route.ts`. O reenvio
não reaproveitou — reinventou. Agora existe `corpoParaOCrm()` em `lib/crm-envio.ts`, e
os dois caminhos chamam ela. Enquanto a regra vivesse dentro da rota, o próximo caminho
que mandasse lead ao CRM erraria de novo.

**2. O botão nunca esteve na tela.** `app/leads/page.tsx` importava `CrmStatusLead` e
declarava `<th>CRM</th>`, mas **a célula não existia**: 7 cabeçalhos, 6 células. O commit
`053e410` (13/08) que criou a funcionalidade tocou o painel em só duas linhas — o import
e o cabeçalho. Ou seja, ninguém nunca viu o selo "não chegou no CRM" nem o botão
"Reenviar". Consertar só o item 1 seria consertar código morto.

**Também:** o timeout do reenvio subiu de 10s pra 20s, igual à captura. O CRM já foi
medido acima de 8s, e é justamente quando ele está lento que alguém aperta "Reenviar".

**Não testado de ponta a ponta:** o reenvio real exige chave do CRM, que não existe em
dev (`chaveDoSlug` devolve `null` e a action para antes do envio). O teste que fecha é em
produção, num lead com `crmStatus: "falhou"`. A coluna CRM na tela também não foi vista
por mim — o painel exige sessão de admin e não consegui autenticar no dev.

Commit `7747d21`. 441 testes, `tsc` limpo, build limpo.

---

## 🎯 As perguntas que dizem quem ligar primeiro (10/09)

**O que mudou na tela.** O formulário da `/transforma` trocou as 2 perguntas de
qualificação (`prontidao_proximo_passo`, `barreira_proximo_passo`) por 4, na ordem que
o comercial usa pra decidir a abordagem:

| `name` | Pergunta |
|---|---|
| `incomodo_atual` | O que mais te incomoda em continuar como está? |
| `quando_comecar` | Quando gostaria de começar a perceber uma mudança? |
| `adiar_decisao` | O que ainda te faria adiar essa decisão? |
| `proximo_passo` | Diante da formação certa, o que você faria? |

Três opções cada, todas obrigatórias. O comportamento é o mesmo de antes: o bloco só
aparece depois que nome + e-mail + WhatsApp estão válidos, e o CSS/JS já eram genéricos
(`.pergunta`, `querySelectorAll` nos rádios) — não precisaram de nada.

**A parte que não aparece na tela — e era a armadilha.** A lista dessas perguntas estava
copiada em TRÊS lugares: o corpo enviado ao CRM (`montarCorpoTransforma`), a coluna
"Perfil" do `/leads` e o cabeçalho do CSV em `/leads/export`. A ponte do formulário e o
`/api/elementor-form` passam campo novo automaticamente (FormData cru + blocklist), mas
esses três eram allowlist hard-coded: pergunta nova no HTML era coletada e **descartada
em silêncio** nos três. Agora existe `PERGUNTAS_TRANSFORMA` em `lib/crm-envio.ts` como
fonte única (`campo` + `rotulo` do painel + `colunaCsv`), e os três lugares mapeiam sobre
ela. Mexeu no formulário, mexe só ali.

**Por que as 2 antigas continuam na lista.** Quem se inscreveu antes de 10/09 tem
`prontidao_proximo_passo`/`barreira_proximo_passo` gravado em `respostas`. Tirar da lista
apagaria esse histórico do painel e da planilha.

**⚠️ Ao criar campo novo**, fugir das substrings que o `pick()` do `/api/elementor-form`
usa como atalho (`nome`, `email`, `mail`, `tel`, `fone`, `phone`, `celular`): um `name`
que as contenha é sequestrado e vira o contato do lead.

**Pendência conhecida** (pré-existente, não introduzida aqui):
`app/leads/reenviar-action.ts` descarta TODAS as respostas de qualificação no reenvio
manual ao CRM e não manda a `tag: "JAY Transforma"` — só nome/email/telefone/pagina.

Commit `0acef64`. 438 testes, `tsc` limpo, build limpo, submit de ponta a ponta testado no
dev (slug `transforma`, telefone normalizado em E.164, as 4 respostas gravadas) e HTML
verificado em produção 70s após o push.

---

## 📈 A página do evento passa a medir (10/09)

**O problema.** A `/transforma` (JAY TRANSFORMA, evento gratuito de 3 dias) estava no
ar sem Meta Pixel e sem GTM. Só levava GA4, a meta de verificação do Facebook, o
forwarder de UTM e o beacon de visita. A campanha do evento não via nem a visita nem
a inscrição — sem PageView, sem público de remarketing, sem conversão pra otimizar.

**O que foi feito.** Nada de código novo: os dois IDs pedidos já eram os do portal
(pixel `1841776429524244`, container `GTM-TVLJSVJZ` do marketing, o mesmo da
`/magicshadow`). Bastou ligar a política por página em dois mapas:

- `GTM_BY_SLUG` (`lib/google-tag.ts`) ganhou `transforma: "GTM-TVLJSVJZ"`;
- `PIXEL_SLUGS` (`lib/meta-tracking.ts`) ganhou `"transforma"`.

**Por que NÃO colar o snippet no HTML.** O `withTracking` limpa antes de injetar:
`stripGoogleTagManager` apaga qualquer script `gtm.js`/`noscript ns.html` e
`stripPixelInits` apaga `fbq('init', …)` embutido. Snippet colado à mão em
`lp-html/transforma.html` sumiria ao servir. E o bootstrap do portal é melhor que o
snippet cru do painel do Meta: gera `eventID` por visita e dispara o CAPI com o mesmo
id, deduplicando no Events Manager.

**O que a página passou a mandar:** `PageView` + `ViewContent` (o `serveLp` passa
`isProductPage: true` fixo pra toda LP de disco — decisão de manter, sem exceção),
`Lead` no submit do formulário de inscrição, e `WhatsApp`/`InitiateCheckout` nos
cliques. Verificado que a ponte de formulários (`montarGuardaDeFormularios`) delega no
`document`, **não** marca `data-portal-bound="1"` e **não** chama `stopPropagation` —
então o listener de Lead anexa normalmente e não há disparo duplicado.

**Nota de infra:** o `GITHUB_PAT` não está mais no `.env.local`; o push como dono sai
pelo helper já configurado no repo (`.git/credential-mktjamesolaya2`).

Commit `43bf71f`. 436 testes, `tsc` limpo, verificado em produção 70s após o push.

---

## 🎯 A origem da venda deixa de morrer no clique (03/09)

**O problema.** As campanhas do Facebook traziam gente pra LP com
`?utm_source=facebook&utm_campaign=...`, mas o checkout é da Hotmart, em outro
domínio, e os links de compra eram estáticos. Quem clicava chegava lá sem nenhuma
marca de onde tinha vindo — o relatório da Hotmart não sabia qual campanha gerou a
venda, e não dava pra fechar a conta entre o que se gasta no anúncio e o que entra.

**O que foi feito.** Um script client-side (`lib/utm-checkout.ts`) reescreve o `href`
de todo link `hotmart.com` da página, anexando:

- os 5 `utm_*` como vieram na URL;
- `src` = a fonte higienizada (aparece na origem da venda);
- `sck` = `fonte-campanha-anúncio` (aparece no relatório de vendas).

Nunca sobrescreve parâmetro que já esteja escrito no link — `off`, `checkoutMode`, ou
um `src` de parceiro continuam intactos.

**Por que client-side.** As rotas das LPs são `force-static`: o servidor nunca vê a
querystring do visitante. Mesmo motivo do beacon de visita e dos listeners do Pixel.

**Detalhes que importam:**

- **Injetado em `withTracking()`** (`lib/meta-tracking.ts`), *fora* do `if` do Pixel —
  vale em toda página, com Meta ou sem. Cobre de uma vez as 8 LPs de `lp-html/`
  (Shadow PRO, inmersion com 8 links, nanofios, magic shadow, lips sense, fio a fio,
  profissão remove) **e** as páginas publicadas do WP, além de qualquer link novo.
- **Reescreve o `href` no DOM**, não intercepta o clique: assim clique do meio, "abrir
  em nova aba" e copiar link também levam a origem. Sem `preventDefault`, então não
  briga com o listener de `InitiateCheckout` que já existia.
- **`sessionStorage['jay:utm']`** guarda os UTMs da entrada, pra que um refresh ou uma
  navegação que limpe a URL não apaguem a origem. Só pela aba — nada de cookie.
- **Idempotente de propósito** (sem flag de "já processado") + `MutationObserver` com
  debounce: conserta link que outro script tenha trocado depois, o que importa nas LPs
  de export Elementor, com 60-80 scripts mexendo no DOM.
- **Higienização** de `src`/`sck`: o Facebook manda macro (`{{ad.name}}`) e nome com
  espaço e acento; vira `[A-Za-z0-9_.-]`, cortado em 100 chars.

**Como a lógica é testada de verdade.** A função que monta a URL vive como *string de
fonte JS* (`MONTAR_URL_JS`) — o mesmo texto que roda no browser é o que o teste
executa via `new Function`. Uma fonte de verdade só, sem a duplicação que existe hoje
entre `lib/telefone.ts` e o JS de `lp-html/transforma.html`.

**Verificado** com Chrome real (puppeteer): Shadow PRO com campanha completa; mesma
aba sem querystring (sessionStorage segurou); aba nova sem UTM (link sai limpo, sem
`src`/`sck` inventado); os 8 links do inmersion, incluindo o `sticky-cta`; e a LP
Elementor com os 15 formulários intactos. 436 testes, `tsc` limpo, build OK.
...
### ✨ Dobra 2 — "Sua melhor versão Hoje!" (20/08)

James: *"vamos para a proxima dobra q é essa, da uma lida nela, que ela tem uns
efeitos especiais"*.

Fui ler a dobra no site que está no ar. Os "efeitos especiais" são três, e
todos vieram — refeitos, não copiados:

| no site antigo | aqui |
|---|---|
| *Animated Headline* do Elementor: círculo à mão no "Hoje!", `loop: yes`, repete a cada 8s | traço em SVG que se desenha **uma vez**, quando a dobra entra |
| 4 colunas com `fadeInUp` | os mesmos 4 cards subindo em fila (atraso de .09s entre eles) |
| 4º card com fundo coral cheio, ícone e texto brancos | 4º card em **ouro** cheio, ícone e texto pretos |

**Os ícones são os MESMOS arquivos do site** (calendário, lábios, sobrancelha,
brilhos) — extraídos do HTML, limpos do lixo do svgjs e com a cor trocada por
`currentColor`. Desenhar ícone novo aqui seria inventar o que já existe.

⚠️ **O traço não repete.** Em laço de 8 em 8 segundos vira tique nervoso.

⚠️ **Como o círculo abraça a palavra**: `preserveAspectRatio="none"` (pra ele
esticar até a largura do "Hoje!") + `vector-effect="non-scaling-stroke"` (pra a
espessura NÃO engordar junto). Sem o segundo, o rabisco vira borda oval. E ele
é maior que a palavra de propósito — traço colado lê como caixa, não caneta.

⚠️ **No celular a quebra é da LINHA, com `<br>`** — não `display:block` na
...
### 🎥 Dobra 4 — depoimentos (20/08)

Os dois vídeos do site antigo: **Nadia Brasil** (`pKtGVSFXOXU`) e a
**reconstrução total fio a fio** (`lgbTFOMLYOk`), no mesmo padrão de moldura da
página.

⚠️ **O YouTube não carrega junto com a página.** Dois embeds trazem ~1,5 MB de
script do Google e três domínios de terceiro em TODA visita, inclusive de quem
nunca dá play. O que entra é a **capa** (a miniatura, baixada e servida por
nós) e o iframe **só nasce no clique** — com `autoplay=1`, senão ela precisa
clicar duas vezes.

⚠️ **O véu escuro por cima da capa é um `::after` do botão** — ele sobrevive à
troca pelo iframe e ficaria escurecendo o vídeo tocando. Sai junto no play
(`.video[data-aberto]::after { display: none }`).
...
### 🙏 A dobra do James virou "o nome atravessando" (21/08)

Ele escolheu a **opção 4** da folha: o nome em letra gigante **atrás** do
retrato — o mesmo gesto da abertura. A página abre com ele e fecha com ele.

⚠️ **A letra é a MESMA da abertura**: caixa baixa, espaço fechado, peso 200. Em
caixa alta ele já testou e voltou atrás — ver a nota de 20/08.

⚠️ **A dobra deixou de ser clara.** A foto é sobre preto; numa lâmina off-white
ela viraria um selo cinza no meio do claro.

**A foto**: ele mandou uma **referência de pose** (mãos unidas à frente do
rosto) e pediu Higgsfield. Gerada com o **retrato de estúdio dele** como
referência de identidade — rosto, óculos, cabelo e barba conferidos contra o
original; só pose e roupa mudaram.

⚠️ **Ela entra RECORTADA, sem fundo.** Com o retângulo aparecia a moldura: o
cinza do estúdio é mais claro que o preto da página. Recortado, o nome
atravessa em volta dele, que é justamente o efeito escolhido.

A ficha virou **fileira** no rodapé da dobra (em coluna brigaria com o retrato
pelo centro) e vira 2×2 no celular.

### ✅ ESTADO DA `/clinica` — fechada e em produção (21/08)

A página está no ar em **jayacademy.com.br/clinica**, com **sete dobras**:

```
abertura (nome em lettering + James recortado)
sua melhor versão (4 cards, traço à mão no "Hoje!")
os procedimentos (lâmina clara + pigmento escorrendo)
a equipe (recortes no chão de ouro) ......... com MARCADOR
depoimentos (2 vídeos, sem carregar o YouTube)
invista em você (foto do lounge + endereço)
quem está por trás (cartão do James)
```

**Verificado antes de fechar**: `npx tsc --noEmit` limpo, **410 testes passando**,
os dois domínios respondendo 200 com a versão nova, e a página inteira
conferida em print no desktop e no celular.

**As três coisas que faltam** — todas dependem de decisão ou material dele:

1. **Fotos da equipe.** A dobra está com silhueta e a palavra "Nome". Precisa de
   4 fotos com fundo liso e claro (pra recortar), corpo até a cintura e a MESMA
   distância de câmera.
2. **A dobra da estrutura.** Dez caminhos desenhados e nenhum escolhido — três
   deles dão pra fazer sem foto nova.
3. **O `noindex`.** É uma linha. ⚠️ Não tirar antes das fotos da equipe: hoje o
   Google indexaria "Nome / Especialidade" numa página de clínica de verdade.

⚠️ No catálogo do painel ela está como **rascunho** de propósito, pelo mesmo
motivo. Virar "publicada" é o mesmo gesto de tirar o `noindex`.
...
O recorte dele **de pé** e o painel de identificação encostando pela direita,
com o filete de ouro sobre o nome — a pegada da referência que ele mandou.

⚠️ **O cartão SOBREPÕE a foto.** Na primeira montagem ele só encostava, porque o
recorte estava centralizado na coluna e sobrava um vão. Encostado, viram duas
coisas lado a lado; **sobreposto, viram uma peça só** — que é o que a referência
faz. A correção foi `justify-self: end` na foto + margem negativa maior no
cartão.

⚠️ **A foto foi REFEITA pra essa planta**: a anterior era busto sentado; a nova é
em pé, do quadril pra cima, com ele deslocado pra esquerda e **espaço à direita**
— que é onde o cartão entra. Mesma referência de identidade (o retrato de
estúdio) e a mesma pose que ele escolheu.

⚠️ **No celular o cartão desce pra baixo da foto**, sem sobreposição: por cima
dela ele cobriria o rosto, que é a única coisa que a dobra tem pra mostrar.

Sobraram **duas variações da foto em pé** geradas e não usadas, caso ele queira
comparar.
...
quatro dobras pretas seguidas.

⚠️ **O texto do site antigo era uma parede de adjetivo** — *"ganhador de muitos
prêmios internacionais"*, *"muito famoso pela arte e delicadeza"*. Virou
**ficha**: formação, especialidade, técnica própria, o que ele treina, o que
dirige. Quem lê confere uma ficha; adjetivo ela desconta. Sobrou uma frase de
crédito (*"melhor micropigmentador do ano por votação do público"*), que é deles
e é atribuível.

⚠️ **A foto é a REAL, sem retoque de IA** — o retoque ficou na abertura. E é
recorte de **busto**: na abertura ele aparece de corpo, aqui de perto. Mesma
pessoa, enquadramento diferente, senão a página repete a mesma imagem.
...
frase e o botão ao lado. *"O que podemos fazer aqui? Se olhar no mobile ficou
bem ruim."*

⚠️ **O que quebra ali é a foto**: ela é FUNDO com ponto focal fixo, então no
estreito sobra um ombro. E foto de banco destoa do resto da página, que só tem
foto real. Por isso o fechamento **não tem foto de modelo**.

A página passa a fechar onde abriu: **na casa da clínica**, a mesma imagem do
hero, bem escurecida. Bookend. Como o fundo é `cover` com o texto empilhado por
cima, não há o que quebrar no celular — ele só empilha.
...
**Escolha dele: a 4 — "recortadas no preto".** Os retratos sem fundo, alinhados
na mesma linha do chão, com nome e especialidade embaixo. É o mesmo gesto da
abertura, onde o James está recortado sobre o preto.

⚠️ **A dobra está no ar com MARCADORES**: silhueta sem rosto e a palavra "Nome".
Nada de rosto de banco nem nome de pessoa inventado — é o tipo de coisa que vaza
pro ar sem ninguém perceber. Quando o material chegar, troca-se a silhueta pela
foto e o marcador pelo nome; a dobra não muda.

**O que as fotos precisam ter** (é o que essa opção cobra): fundo liso e claro
pra recortar, corpo até a cintura, e a MESMA distância de câmera nas quatro.

⚠️ **A linha do chão é um item da grade**, não borda de cada foto — como borda
ela sai picotada nos vãos. E as alturas variam 6% entre os quatro: quatro
recortes na mesma altura exata lêem como fileira de crachá.

⚠️ **No celular** a grade de 2 colunas jogava as quatro fotos primeiro e os
quatro nomes depois, cada nome longe da sua foto. Cada um teve que ser posto no
lugar na mão.

### 🕳️ A poça de pigmento no celular

A massa curva da dobra clara passava **por cima do texto de lábios** no celular:
lá não existe área vazia — é uma coluna só e o texto ocupa a largura inteira.
Ela virou uma **poça no pé da dobra**, num respiro criado pra ela, e a curva
virou o topo inclinado da poça. As gotas somem nessa versão: espremidas na
faixa, viravam traços.
...
estou achando muito pequena tb"*.

- **Claro**: tokens invertidos dentro do `.claro`, igual à /academy. ⚠️ **O ouro
  muda de tom no claro** — `#8A7A3E` no lugar de `#AC9751`, que sobre off-white
  quase some. E o **filete dourado entre dobras sai** quando um dos lados é
  claro: ali quem separa é a própria virada de fundo.
- **Fonte**: a escala da /academy é de página **escura**, onde texto fino
  brilha; no claro ela ficou miúda. Parágrafo 15→**17px**, itens 14.5→**16.5px**,
  título 33→**38px**, rótulos e link na mesma proporção.

### 💄 Dobra 3 — os procedimentos (20/08)

"A beleza está nos detalhes", refeita do site antigo. Lá é um **carrossel de
dois slides com seta**; aqui viraram **duas lâminas empilhadas**, alternando o
lado da foto — no carrossel, a segunda só existe pra quem clica na seta.

Padrão da /academy: foto em moldura, título em duas linhas, filete de 54px
abrindo o parágrafo e **link fino sublinhado** no lugar de botão cheio (botão
aqui competiria com o da abertura, que é o que a gente quer que ela clique).

⚠️ **Sem filete entre os itens.** James: *"esse tanto de linhas aqui eu não
gosto, quero algo mais clean, igual de revista"*. O que pesava não era só o
filete — era a **largura**: a coluna de texto ia até o fim da tela e cada linha
virava régua de 1000px. Agora a coluna tem **medida (46ch)** e o que separa os
itens é o respiro.

⚠️ **A lâmina invertida inverte as COLUNAS**, não só a ordem. Só com `order`, a
foto caía na coluna larga e virava pôster ao lado de um texto espremido.

### 📷 As fotos da dobra 3 — e a linha que eu não cruzei

- **Sobrancelhas**: foto **REAL** do acervo (`macro-2.jpg`, o pigmento sendo
  aplicado). O banco tem material bom: sempre olhar antes de gerar.
- **Lábios**: **gerada no Higgsfield** — e de propósito é o **procedimento
  acontecendo**, não um "depois". ⚠️ **Resultado gerado por IA numa página de
  clínica é prova falsa de trabalho.** Se ele quiser um "antes e depois" ali,
  tem que sair do acervo dele.

Por que precisou gerar: o banco só tem "depois" de lábios **com marca d'água de
outra profissional** (Joy Vieira) e em 800×399 — não dá nem em qualidade nem em
direito de imagem. Fica anotado, porque isso vale pra próxima vez que alguém
for buscar foto de lábios aqui.

⚠️ A primeira escolha (`r-labios.jpg`, macro de lábios naturais) foi trocada:
...
Foto com `loading="lazy"` abaixo da primeira tela saía **preta** no print de
página inteira — o navegador não rola de verdade. Isso já me fez achar que uma
imagem estava quebrada quando ela estava perfeita. Agora o print tira o lazy e
espera as imagens chegarem.
...
James: *"ficou feio o james recortado assim (...) adiciona um gradiente preto
nessa divisão"*. O recorte terminava numa linha reta na altura da cintura —
efeito de foto colada. Agora ele se desmancha no preto por `mask-image`.

### 🔧 `npm run foto -- --esperar`

A espera antes do print era **fixa em 600ms**, o suficiente pra entrada padrão
(`.surge`, 700ms). O traço do "Hoje!" leva 1,6s: o print pegava só o começo do
risco e **parecia que tinha quebrado**. Agora dá pra pedir mais tempo.
